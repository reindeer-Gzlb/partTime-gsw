<template>
	<view class="mian" :style="{height: gao + 'px' }">
		...
		<view class="selects-box" style="border-bottom: 1px solid #1C1632;">
			<view class="title" v-if="pageType == 'kefu'">
				请先选择你要咨询的游戏，系统为您匹配客服
			</view>
		...
		methods: {
			...
			gameChange(item) {
				if (this.pageType == 'kefu') {
					uni.navigateTo({
						url: "/pages/homedetails/pingtaikefu?gameId=" + item.id + '&gameName=' + item.name
					})
				} else if (this.pageType == 'jisu') {
					uni.navigateTo({
						url: "/pages/homedetails/pingtaikefu?gameId=" + item.id + '&gameName=' + item.name +
							'&type=jisu'
					})
				} else if (this.pageType == 'jingjia') {
					uni.navigateTo({
						url: "/pages/retrieveAccount/jingjiahuishou/inputInfo?type=" + item.id + '&gameId=' + item
							.id +
							'&gameType=' + item.type + '&gameName=' + item.name + '&gameIcon=' + item.icon
					})
				} else if (this.pageType == 'guanwang') {
