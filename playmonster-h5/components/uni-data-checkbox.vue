	...
	<view class="uni-data-checklist" :style="{'margin-top':isTop+'px'}">
		...
		<template v-else>
			<checkbox-group v-if="multiple" class="checklist-group" :class="{'is-list':mode==='list' || wrap}"
				@change="chagne">
				<label class="checklist-box" v-if="item.hsisShow==1"
	...
	export default {
		...
		methods: {
			...
			setStyleBackgroud(item) {
				let styles = {}
				let selectedColor = this.selectedColor ? this.selectedColor : '#71A5FF'
				if (this.selectedColor) {
					if (this.mode !== 'list') {
						styles['border-color'] = item.selected ? selectedColor : '#ddd'
					}
					if (this.mode === 'tag') {
			...
			setStyleIcon(item) {
				...
				if (this.selectedColor) {
					let selectedColor = this.selectedColor ? this.selectedColor : '#71A5FF'
					styles['background-color'] = item.selected ? selectedColor : 'rgb(36, 40, 104)'
					styles['border-color'] = item.selected ? selectedColor : '#DCDFE6'

					if (!item.selected && item.disabled) {
						styles['background-color'] = 'rgb(36, 40, 104)'
						styles['border-color'] = item.selected ? selectedColor : '#DCDFE6'
					}
				}
				for (let i in styles) {
