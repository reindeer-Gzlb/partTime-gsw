  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:recovery:add']"
        >入库</el-button>
      </el-col>
      <!--<el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleDelete"
          v-hasPermi="['system:recovery:audit']"
        >审核包赔</el-button>
      </el-col>
      <!--<el-col :span="1.5">
        <el-button
          ...
          v-hasPermi="['system:recovery:edit']"
        >编辑</el-button>
      </el-col>-->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="审核" prop="recoveryState">
          <el-radio-group v-model="form.recoveryState">
            <el-radio label="1">通过</el-radio>
            <el-radio label="2">不通过</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="审核备注" prop="recoveryNote">
          <el-input v-model="form.recoveryNote" type="textarea" placeholder="请输入内容" />
        </el-form-item>
      </el-form>
    ...
    <el-dialog :title="auditTitle" :visible.sync="auditOpen" width="500px" append-to-body>
        ...
        <el-form-item label="系统id:" v-model="auditForm.id">{{auditForm.id}}</el-form-item>
        <el-form-item label="url地址:" v-model="auditForm.url">{{auditForm.url}}</el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary"
                   ...
                   size="mini"
                   @click="submitAuditForm">确 定</el-button>
        <el-button type="primary"
                   plain
                   icon="el-icon-success"
                   size="mini"
...
<script>
import { listInfo, getInfo,  auditInfo, updateInfo, exportInfo,byColums,recoveryEditInfo } from "@/api/recovery/info";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.loading = true;
      this.queryParams.recoveryState = 0;
    ...
    handleAdd: function (row) {
      const flag = row.auditFlag || this.auditFlag;
      if (typeof flag == "Array"){
        if (flag.length !=0){
          if (flag[0]== 0){
            this.$modal.msgWarning("当前账号已在审核包赔流程，无法操作!!");
            return;
          }
        }
      }else {
      ...
      accountFrom.recoveryState = 1;
      accountFrom.saleState = 0;
      accountFrom.recoveryNote = "";
      updateInfo(accountFrom).then(response => {
    ...
    handleUpdate(row) {
      ...
      getInfo(id).then(response => {
        ...
        this.form.recoveryState = null;
        this.form.recoveryNote = null;
        this.open = true;
        this.title = "审核游戏账号";
      });
    },
    ...
    submitrecoveryNoteForm() {
      this.$refs["recoveryNoteForm"].validate(valid => {
        if (valid) {
          //不需要新增，只需要审核
          recoveryEditInfo(this.recoveryNoteForm).then(response => {
            this.$modal.msgSuccess("提交成功");
            this.showrecoveryNoteVisible = false;
            this.getList();
          });
    ...
    handleDelete() {
      ...
      this.auditForm.url=this.hoste+"?id="+ids[0];
      this.auditOpen = true;
