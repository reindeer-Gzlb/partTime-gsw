...
<script>
import addDialog from "./addSh.vue";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$nextTick(() => {
        financeApi
          .getFinanceListSh(this.queryParams)
          .then(response => {
            this.$nextTick(() => {
              this.loading = false;
    ...
    // 获取账单类型列表
    getTypeList() {
      financeApi.getBillTypeCW().then(res => {
    ...
    delItem(row) {
      // 角色列表中含有【22】时无需密码
      // 获取登录用户的角色列表
      let loginRoleList = this.$store.state.user.roles;
      if (loginRoleList.indexOf('22') != -1) {
