...
public class SteamService {
    ...
    public JSONObject getStr(String url) {
        ...
        try {
            ...
            result= HelpUnauthorizedLoginCanRecover(sessiodId, stoken, proxy);
            result.put("code","200");
            result.put("msg","执行成功");
        } catch (IOException e) {
            result.put("code","301");
            result.put("msg","IOException 异常");
        } catch (RuntimeException e) {
            result.put("code","408");
            result.put("msg","RuntimeException 运行超时");
        }catch (Exception e){
            result.put("code","500");
            result.put("msg","Exception 系统错误");
        }
